...
namespace Einsatzueberwachung.Services
{
    public class PersistenceService
    {
        ...
        private string _configFileName = "einsatzueberwachung.json";
        ...
        public void Initialize(string configFileName, bool enableAutoSave)
        {
            _configFileName = configFileName;
            _autoSaveEnabled = enableAutoSave;
            LoggingService.Instance.LogInfo($"PersistenceService initialized with config: {configFileName}, AutoSave: {enableAutoSave}");
        }
        ...
        private bool HasDataChanged(EinsatzSessionData newData)
        {
            ...
            for (int i = 0; i < Math.Min(_lastSavedData.Teams.Length, newData.Teams.Length); i++)
            {
                ...
                if (oldTeam.IsRunning != newTeam.IsRunning ||
                    ...
                    oldTeam.Suchgebiet != newTeam.Suchgebiet)
                {
                    return true;
                }
            }
    ...
    public class TeamSessionData
    {
        ...
        public string Suchgebiet { get; set; } = string.Empty;  // ← DIESE ZEILE HINZUFÜGEN
